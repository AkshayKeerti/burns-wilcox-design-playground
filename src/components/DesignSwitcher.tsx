...
export default function DesignSwitcher() {
  ...
  const handleDesignChange = (designId: string, designPath: string) => {
    ...
    if (designPath) {
      ...
      const currentPath = pathname.replace('/design-1', '');
      router.push(`${designPath}${currentPath}`);
    } else {
      // For original design, remove design prefix
      const cleanPath = pathname.replace('/design-1', '');
  ...
  const getCurrentDesignName = () => {
    const current = designOptions.find(option => 
      pathname.includes(option.path) || (option.path === '' && !pathname.includes('/design-1'))
    );
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-corporate-grey-100 z-50">
          <div className="p-4">
